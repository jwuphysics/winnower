...
def test_cli_help():
    ...
    result = subprocess.run([
        sys.executable, "-m", "winnower.cli", "--help"
...
def test_setup_command_smoke(mock_check, mock_setup):
    ...
    result = subprocess.run([
        sys.executable, "-m", "winnower.cli", "setup"
    ], capture_output=True, text=True)
...
def test_file_processing_smoke(mock_openai, sample_ml_paper, temp_dir):
    ...
    output_files = list(temp_dir.glob("*_technical_summary.md"))
