...
def setup_command(args) -> int:
    ...
    print("\nAPI Key Status:")
    print(f"  OpenAI: {'✓ Found' if api_status['openai'] else '✗ Not found'}")
    print(f"  Anthropic: {'✓ Found' if api_status['anthropic'] else '✗ Not found'}")
    
    if not any(api_status.values()):
        print(f"\n⚠️  No API keys found. Please edit {env_path} and add your API key.")
