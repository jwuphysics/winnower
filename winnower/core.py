    ...
    def process(self, input_source: str, output_dir: Path, recursive: bool = False) -> None:
        ...
        for paper_source in papers:
            try:
                ...
                extracted_filename = self._generate_safe_filename(paper_data['title'], "extracted")
                extracted_file = extracted_dir / f"{extracted_filename}.md"
                extracted_file.write_text(paper_data['content'], encoding='utf-8')
                if self.verbose:
                ...
                summary_filename = self._generate_safe_filename(paper_data['title'], "summary")
